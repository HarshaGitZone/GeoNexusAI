...
# # --- INTERNAL ANALYSIS LOGIC PRESERVED ---
# def _perform_suitability_analysis(latitude: float, longitude: float) -> dict:
#         # (1. Water Early Exit, 2. Forest Check, 3. Land Analysis, 4. Ensemble Logic all remain exactly as you pasted)
#         w_score, w_dist, w_meta = estimate_water_proximity_score(latitude, longitude)
#         w_score = round(w_score, 2) if w_score else 0.0
...
#                 "factors": {k: 0.0 for k in ["rainfall", "flood", "landslide", "soil", "proximity", "water", "pollution", "landuse"]},
#                 "reason": f"Location is on {water_name}. Unsuitable for construction.",
#                 "explanation": {"factors_meta": {"water": {"reason": f"Directly on {water_name}", "source": "Satellite", "confidence": "High"}}},
#                 "evidence": {"water_distance_km": 0.0, "water_details": w_meta},
#                 "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S IST")
#             }

#         landuse_result = infer_landuse_score(latitude, longitude)
#         landuse_s = landuse_result[0] if isinstance(landuse_result, tuple) else landuse_result
#         landuse_s = round(landuse_s, 2) if landuse_s else 70.0
...
#                 "label": "Not Suitable (Protected/Forest Area)",
#                 "factors": {k: 0.0 for k in ["rainfall", "flood", "landslide", "soil", "proximity", "water", "pollution", "landuse"]},
#                 "reason": "Location is in a forest area.",
#                 "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S IST")
#             }
...
#         rainfall_reason = f"Rainfall: {rain_mm}mm logic applied."
#         pollution_reason = f"PM2.5: {poll_value} logic applied."
#         soil_explanation = f"Soil quality score: {soil_s}/100."
...
#             model_used = "Ensemble"
#         except:
#             agg = compute_suitability_score(rainfall_score=rainfall_score, flood_risk_score=flood_s, landslide_risk_score=landslide_s, soil_quality_score=soil_s, proximity_score=prox_s, water_proximity_score=w_score, pollution_score=poll_s, landuse_score=landuse_s)
#             final_score = agg.get("score")
#             model_used = "Weighted Aggregator"

#         return {
#             "suitability_score": final_score,
#             "label": "Highly Suitable" if final_score >= 70 else ("Moderate" if final_score >= 40 else "Unsuitable"),
#             "model_used": model_used,
#             "factors": {"rainfall": rainfall_score, "flood": flood_s, "landslide": landslide_s, "soil": soil_s, "proximity": prox_s, "water": w_score, "pollution": poll_s, "landuse": landuse_s},
#             "explanation": {
#                 "factors_meta": {
#                     "water": {"reason": "Analyzed.", "source": "Map Engine", "confidence": "High"},
#                     "rainfall": {"reason": rainfall_reason, "source": "Meteorological Data", "confidence": "High"},
#                     "flood": {"reason": "Risk assessed.", "source": "Integrated Models", "confidence": "High"},
#                     "landslide": {"reason": f"Slope stability score: {landslide_s}.", "source": "NASA SRTM", "confidence": "Medium"},
#                     "soil": {"reason": soil_explanation, "source": "Soil Survey", "confidence": "Medium"},
#                     "proximity": {"reason": "Distance analyzed.", "source": "OpenStreetMap", "confidence": "High"},
#                     "pollution": {"reason": pollution_reason, "source": "Satellite Data", "confidence": "High"},
#                     "landuse": {"reason": "Classification applied.", "source": "Sentinel-2", "confidence": "High"}
#                 }
#             },
#             "evidence": {"water_distance_km": w_dist, "rainfall_total_mm_60d": rain_mm},
#             "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S IST"),
