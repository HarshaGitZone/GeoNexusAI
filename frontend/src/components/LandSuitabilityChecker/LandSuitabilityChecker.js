...
//     ? "https://geoai-major-vnrvjiet22-26-itb.onrender.com":"http://localhost:5000" ;
...
//     let name = "";
//     if (isNearbyDevice(lat, lng, deviceLocation)) {
//         name = "My Location";
...
//       if (!res.ok) {
//         if (res.status === 503) alert("Server waking up. Please wait.");
//         return;
//       }
...
//       setNearbyData(data);
//       setShowNearby(true);
//     } catch (err) { console.error("Nearby A Error:", err); } finally { setNearbyLoading(false); }
//   };
...
//       setNearbyDataB(data);
//       setShowNearby(true);
//     } catch (err) { console.error("Nearby B Error:", err); } finally { setNearbyLoadingB(false); }
//   };
...
//     const matchedPlaceB = savedPlaces.find(p => 
//         p.lat.toFixed(4) === curLatB && p.lng.toFixed(4) === curLngB
//     );

//     let name = existingName;
//     if (!name) {
//         if (isNearbyDevice(tLat, tLng, deviceLocation)) name = "My Location";
//         else if (matchedPlaceB) name = matchedPlaceB.name;
//         else name = prompt("Enter name for Location B:") || "Site B";
//     }
...
//     setIsBFromSavedPlace(!!existingName || !!matchedPlaceB);
//     try { 
...
//       setBLatInput(bLat.toString());
//       setBLngInput(bLng.toString());
//       handleCompareSelect(bLat, bLng);
//     });
//   };
...
//                 />
//               </div>
//               <div className="col-2"><EvidenceSection data={result} /></div>
//             </div>
//           )}
...
//                 </div>
//                 <div className="compare-pane-ditto">
//                     <h4 className="pane-header">{compareName.toUpperCase()}</h4>
//                     {compareResult ? (
//                         <>
//                         <FactorsSection 
//                           data={compareResult} latVal={bLatInput} lngVal={bLngInput} locationName={compareName || "Site B"}
//                           isDarkMode={isDarkMode} viewMode={viewMode} setViewMode={setViewMode} onOpenHistory={handleOpenHistory} mapVariety={mapVariety}
//                         />
...
//       {/* NEARBY PLACES MODAL UI */}
//       {showNearby && (
//           <div className="modal-overlay" onClick={() => setShowNearby(false)}>
//             <div className="modal-card" onClick={(e) => e.stopPropagation()}>
//               <div className="modal-header">
//                 <h3>Nearby Amenities (1.5 km)</h3>
//                 <button className="modal-close" onClick={() => setShowNearby(false)}>✖</button>
//               </div>

//               <div className="modal-body">
//                 {(nearbyData?.places?.length || nearbyDataB?.places?.length) ? (
//                   (() => {
//                     const activeData = nearbyData || nearbyDataB;
//                     const schools = activeData.places.filter(p => p.type === "school");
//                     const hospitals = activeData.places.filter(p => p.type === "hospital");
//                     const colleges = activeData.places.filter(p => p.type === "college" || p.type === "university");
...
//                     return (
//                       <>
//                         <NearbySection title="🏫 Schools" items={schools} />
//                         <NearbySection title="🏥 Hospitals" items={hospitals} />
//                         <NearbySection title="🎓 Colleges & Universities" items={colleges} />
//                       </>
//                     );
//                   })()
//                 ) : (
...
//     // Set up states for simultaneous analysis if showLocationB is active
//     setResult(null); 
//     setCompareResult(null); 
//     setLoading(true);
//     if (showLocationB) {
...
//     // If B is visible, analyze it too
//     if (showLocationB && bLatInput && bLngInput) {
//         const curLatB = parseFloat(bLatInput).toFixed(4);
//         const curLngB = parseFloat(bLngInput).toFixed(4);
...
//     const isDeviceLocationB = curLatB === deviceLocation.lat && curLngB === deviceLocation.lng;
//     const matchedPlaceB = savedPlaces.find(p => 
//         p.lat.toFixed(4) === curLatB && p.lng.toFixed(4) === curLngB
//     );
...
//     let name = existingName;
//     if (!name) {
//         if (isDeviceLocationB) name = "My Location";
//         else if (matchedPlaceB) name = matchedPlaceB.name;
//         else name = prompt("Enter name for Location B:") || "Site B";
//     }
...
//           {(result || (isCompareMode && (loading || compareLoading))) && !isCompareMode && (
//             <div className="results-grid">
//               <div className="col-1">
//                 <h4 className="pane-header">{locationAName.toUpperCase()}</h4>
//                 <FactorsSection 
...
//           {(result || isCompareMode) && isCompareMode && (
//             <div className="compare-layout-ditto" style={{ display: 'flex', height: '100%', width: '100%' }}>
//                 <div className="compare-pane-ditto">
//                     <h4 className="pane-header">{locationAName.toUpperCase()}</h4>
//                     {result ? (
